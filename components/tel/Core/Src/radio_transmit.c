 ...
 * @return void
*/
static void copy_to_buffer(uint8_t* buffer, uint32_t start_index, uint32_t end_index, const uint8_t* source) {
 ...
 * @return void
*/
static void copy_CAN_id(uint8_t* source, CAN_Radio_msg_t *tx_CAN_msg, uint8_t can_id_type) {
...
void RADIO_TRANSMIT_CAN_msg(CAN_Radio_msg_t *tx_CAN_msg)
{
  ...
  copy_to_buffer(radio_buffer, TIMESTAMP_INDEX_START, TIMESTAMP_INDEX_END, (const uint8_t*)&tx_CAN_msg->timestamp.double_as_int);     /* TIMESTAMP */
