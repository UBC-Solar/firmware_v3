...
{
	RADIO_QueueMsg_TypeDef* current_queue_msg_ptr = &sg_rx_queue[sg_tx_queue_index];

	if (!(current_queue_msg_ptr->is_sent))		
	{
		RADIO_Msg_TypeDef* radio_msg = &(current_queue_msg_ptr->radio_msg);
...
void RADIO_set_rx_msg(uint32_t can_id, uint8_t* can_data, uint32_t DLC)
{
	...
	if ((current_queue_message->is_sent))		// If the message is not sent, do not overwrite
	{
		...
		current_queue_message->radio_msg.can_id_reversed = BITOPS_32BIT_REVERSE(can_id);
		...
		sg_rx_queue_index = CIRCULAR_INCREMENT_SET(sg_rx_queue_index, MAX_RX_QUEUE_SIZE);		// Move Rx tracker to next position
	}
