...
class Ui_MainWindow():
    ...
    def __init__(self, MainWindow):
        ...
        self.boxes=[] # an array that stores objects of class box_i
        ...
        self.specific_name_of_text_file= "LOGS_"+generate_file_name() + ".txt" #file name to save
    ...
    def update_status_table(self, array, letter):
        ...
        for i in range(len(array)):
            ...
            # message Here, we check if they are deleted. If so, we add them back:
    ...
    def generate_status_table(self):
        ...
        self.status_table.setGeometry(self.relx(10),self.rely(50),self.relx(1300),self.rely(650)) #relative coordinates
        ...
        self.status_table.setHorizontalHeaderLabels(['COMM','ST','OT','UV','OV','NO_VOLT','TEMP_RANGE','','BAL','LLIM',
                                                     'HLIM','CHARGE_OT','LOW_V','HIGH_V','LOW_T','HIGH_T','OC_IN',
                                                     'OC_OUT'])

        self.status_table.clicked.connect(self.table_is_clicked)#If the table is clicked, this function will be executed.
    ...
    def generate_things(self,MainWindow):
        ...
        #The entire function is just setting up variables. There isn't reall anything to see.
        #If you want to see a bunch of settings get configured, go ahead and read the function
        #all variables are set with relative positions (using the self.rely and self.relx functions)
    ...
    def generate_tick_boxes(self):
        ...
        for i in range(16):
            ...
            label.setGeometry(self.relx(920),self.rely(70+i*spacing),self.relx(75),self.rely(15))
        ...
        for i in range(16):
            ...
            # tickbox.hide()
            label = QtWidgets.QLabel(parent = self.MainWindow)
            label.setGeometry(self.relx(1160),self.rely(70+i*spacing),self.relx(75),self.rely(15))
    ...
    def display_main(self):
        ...
        if self.alpha!=0:
            ...
            #The following two functions are not typos. The same function is executed twice on purpose
    ...
    def generate_pack_wide_info(self,MainWindow,textforthis=None):
        ...
        self.pack_wide_voltage = box_info(self.MainWindow,
                                          ...
                                          self.height,
                                          self.width,
                                          text_list=["Pack Wide Voltage: " +str(self.total_voltage)]
                                          )
    ...
    def draw_voltage_graph(self):
        ...
        for i in range(32):
            ...
            if self.check_boxes[i].isChecked():
                ...
                max=-1
                for j in range(1,1+space_for_legend):
                    newxs.append(j-space_for_legend+1+self.num_of_entries)
                    newy_field.append(0)

                for value in self.boxes[i].past_voltages:
                    k += 1
                    xs.append(k+self.num_of_entries)
                    y_field.append(float(value))
                ...
                if min!=100000000000000000000:
                    minstring=str(signi_digs(round(min,1)))
                else:
                    minstring="None"
                ...
                if max==-1:
                    ...
                    maxstring = str(signi_digs(round(max,1)))
                ...
                item=pyqtgraph.PlotCurveItem(x=xs,y=y_field, pen=pyqtgraph.mkPen(self.boxes[i].color,
                                             width=self.rely(5)),name="Module " + str(i+1) +
                                                                      "\nMin:" + minstring + " "+ "Max: " +
                                                                      maxstring)

                self.voltage_graph.addItem(item)
                self.voltage_graph.addItem(invisible_item)
    ...
    def draw_temp_graph(self):
        ...
        for i in range(32):
            ...
            if self.check_boxes[i].isChecked():
                ...
                for value in self.boxes[i].past_temps:
                    k += 1
                    xs.append(k + self.num_of_entries)
                    y_field.append(float(value))
                ...
                if min!=100000000000000000000:
                    minstring=signi_digs(str(round(min,1)))
                else:
                    minstring="None"
                ...
                if max==-1:
                    ...
                    maxstring = signi_digs(str(round(max,1)))
                item=pyqtgraph.PlotCurveItem(x=xs,y=y_field, pen=pyqtgraph.mkPen(self.boxes[i].color,
                                             width=self.rely(5)),name="Module " + str(i+1) +
                                                                      "\nMin:" + minstring + " "+ "Max: " +
                                                                      maxstring)
    ...
    def update_values(self,array_of_values, label_index, letter="V: "):
        ...
        for i in range(len(self.boxes)):
            try:
                ...
                self.boxes[i].labels[label_index].setText(letter+str(signi_digs(round(array_of_values[i],significant_digits)))+ " " +u)
            ...
            if label_index==0:
                ...
                try:self.boxes[i].update_past_voltages(array_of_values[i])
                except:pass

            elif label_index == 1:
                #the input is temperature, we call the box_info method to store this temperature:
                try:self.boxes[i].update_past_temps(array_of_values[i])
                except:pass
    ...
    def thread(self):
        ...
        if yes:
            ...
            self.worker = reader_thread(self.port_name, MainWindow=self.MainWindow,
                                        file_path_to_record=self.directory_to_save+self.specific_name_of_text_file)
        ...
        if 'y' in let or "Y" in let:
            ...
            if value[0] in ['v','V']:
                ...
                for i in range(len(array_form)):
                    try:
                        self.total_voltage+=int(array_form[i])/10000
                        new_value.append(int(array_form[i])/10000)
                    except:pass

                array_form=new_value
                self.pack_wide_voltage.labels[0].setText("Total Voltage: " +str(round(self.total_voltage,significant_digits)))
