...
uint16_t apiPackage(uint8_t raw_outbox[], uint8_t end_position, uint8_t unsized_packet[],uint8_t MessageCount, uint8_t MessageType){
  ...
  for (int i = 0; i <= end_position; i++){
   ...
   unsized_packet[2] = outbox_position - 4; //Length second byte (ignore start delimiter, length fields, checksum (4 bytes)
   uint16_t totalBytes = 0; //used to calculate checksum

  //add up all bytes to calculate checksum
  for(int i =3; i< outbox_position; i++){
      totalBytes += unsized_packet[i];

  }

  uint8_t checksum = 0xFF -(totalBytes & 0xFF);
  unsized_packet[outbox_position] = checksum; //set checksum

  return outbox_position; //used to size packet to not include any empty positions in the array.
